...
def generate_coloredNum(guess: str, answer: str) -> str:
    coloredNum = [EMOJI_CODES["nerdle"]["normal"]["gray"][number] for number in guess]
    ...
    for i in range(len(guessNumbers)):
        if guessNumbers[i] == answerNumbers[i]:
            coloredNum[i] = EMOJI_CODES["nerdle"]["normal"]["green"][guessNumbers[i]]
            answerNumbers[i] = None
            guessNumbers[i] = None
    for i in range(len(guessNumbers)):
        if guessNumbers[i] is not None and guessNumbers[i] in answerNumbers:
            coloredNum[i] = EMOJI_CODES["nerdle"]["normal"]["yellow"][guessNumbers[i]]
...
def NerdleEmbed(user: nextcord.User, puzzle_id: int) -> nextcord.Embed:
    embed = nextcord.Embed(title="Nerdle", timestamp=date, color=0x0000FF)
    embed.description = "\n".join([generate_blanks()] * 6)
    embed.set_author(name=user.name, icon_url=user.avatar.url)
    embed.set_footer(
        text=f"ID: {puzzle_id} ︱ To play, use the command /games nerdle!\n"
...
def randomNerdleEquat() -> int:
    return random.randint(0, len(equats) - 1)

def is_game_over(embed: nextcord.Embed) -> bool:
    return "\n\n" in embed.description

async def NerdleProcessMessageAsGuess(bot: nextcord.Client, message: nextcord.Message) -> bool:
    ...
    if (
        ...
        reply = "Start a new game with /games nerdle"
        if embed.author:
    ...
    if is_game_over(embed):
        await message.reply("The game is already over. Start a new game with /games nerdle", delete_after=5)
        try:
            await message.delete(delay=5)
        except Exception:
            pass
    ...
    if len(guess) == 0:
        await message.reply(
            ...
            f"To bypass this restriction, you can start a game with `@\u200b{bot_name} play` instead of `/games nerdle`",
            delete_after=14,
        )
        try:
